    ...
    if (ValidarCampos()) {
        ...
        $.ajax({
            ...
            if (data.codigo === "Error") {
                ...
                $("#divListaDocumento").html('<h4 class="card-title" style="text-align:center">Sin resultados para mostrar</h4>');
                Ocultar();
            }
...
function ValidarCampos() {
    ...
    if (!Valido) {
        swal({
            title: 'Informaci\u00f3n',
            text: 'Es obligatorio capturar Documento',
            icon: 'warning',
...
function Buscar() {
    ...
    $.ajax({
        url: '/Catalogos/Documento/Buscar',
        type: "POST",
        data: { Documento, Estatus }
    }).done(function (data, textStatus, jqXHR) {
        if (data.codigo === "Error") {
            resetToastPosition();
            $.toast({
            ...
            $("#divListaDocumento").html(data.ListaCat);
            $('#tblDocumento').DataTable({
                ...
                "language": {
                    ...
                    "zeroRecords": "Ningun Documento Encontrado - valide su busqueda",
                    "info": "Mostrando página _PAGE_ de _PAGES_",
                    "infoEmpty": "No hay Documentos disponible",
...
function Editar(Id, Documento, Estatus) {
    $("#txtIdDocumentoMEditar").val(Id);
    $("#txtDocumentoMEditar").val(Documento);
    if (Estatus == "Activo") {
    ...
    $("#txtDocumentoMEditar").focus();
}

function EditarDocumento() {
    if (ValidarCamposEditar()) {
        var Id = parseInt($("#txtIdDocumentoMEditar").val());
        var Documento = $("#txtDocumentoMEditar").val();
        ...
        $.ajax({
            url: '/Catalogos/Documento/Editar',
            type: "POST",
            data: { Id, Documento, Estatus }
        }).done(function (data, textStatus, jqXHR) {
            if (data.codigo === "Error") {
                resetToastPosition();
                $.toast({
...
function ValidarCamposEditar() {
    var Valido = true;
    var dato = $("#txtDocumentoMEditar").val();
    if (dato === "") {
        Valido = false;
    }
    if (!Valido) {
...
function Eliminar(Id, EstatusSTR) {
    if (ValidarEliminar(EstatusSTR)) {
        ...
        $.ajax({
            url: '/Catalogos/Documento/Eliminar',
            type: "POST",
            data: { Id }
        }).done(function (data, textStatus, jqXHR) {
            if (data.codigo === "Error") {
