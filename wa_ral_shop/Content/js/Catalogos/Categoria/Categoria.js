function AltaCategoria() {
    if (ValidarCampos()) {
        var Categoria = $("#txtCategoriaM").val();
        resetToastPosition();
        //var ToastInfo =
        $.toast({
        ...
        $.ajax({
            ...
            if (data.codigo === "Error") {
                ...
                $("#divListaCategoria").html('<h4 class="card-title" style="text-align:center">Sin resultados para mostrar</h4>');
                Ocultar();
            }
...
function ValidarCampos() {
    ...
    if (!Valido) {
        swal({
            title: 'Informaci\u00f3n',
            text: 'Es obligatorio capturar categoria',
            icon: 'warning',
...
function Buscar() {
    ...
    $.ajax({
        url: '/Catalogos/Categoria/Buscar',
        type: "POST",
        data: { Categoria, Estatus }
    }).done(function (data, textStatus, jqXHR) {
        if (data.codigo === "Error") {
            resetToastPosition();
            $.toast({
            ...
            $("#divListaCategoria").html(data.ListaCat);
            $('#tblCategoria').DataTable({
                ...
                "language": {
                    ...
                    "zeroRecords": "Ninguna Categoria Encontrada - valide su busqueda",
                    "info": "Mostrando página _PAGE_ de _PAGES_",
                    "infoEmpty": "No hay categorias disponible",
...
function Editar(Id, Categoria, Estatus) {
    $("#txtIdCategoriaMEditar").val(Id);
    $("#txtCategoriaMEditar").val(Categoria);
    if (Estatus == "Activo") {
    ...
    $("#txtCategoriaMEditar").focus();
}

function EditarCategoria() {
    if (ValidarCamposEditar()) {
        var Id = parseInt($("#txtIdCategoriaMEditar").val());
        var Categoria = $("#txtCategoriaMEditar").val();
        ...
        $.ajax({
            url: '/Catalogos/Categoria/Editar',
            type: "POST",
            data: { Id, Categoria, Estatus }
        }).done(function (data, textStatus, jqXHR) {
            if (data.codigo === "Error") {
                resetToastPosition();
                $.toast({
...
function ValidarCamposEditar() {
    var Valido = true;
    var dato = $("#txtCategoriaMEditar").val();
    if (dato === "") {
        Valido = false;
    }
    if (!Valido) {
...
function Eliminar(Id, EstatusSTR) {
    if (ValidarEliminar(EstatusSTR)) {
        ...
        $.ajax({
            url: '/Catalogos/Categoria/Eliminar',
            type: "POST",
            data: { Id }
        }).done(function (data, textStatus, jqXHR) {
            if (data.codigo === "Error") {
